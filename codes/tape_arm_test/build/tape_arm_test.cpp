...
void loop() 
{
  ...
  while (!stopbutton())
  {
    ...
    {
      artifact_number++;
      motor.speed(arm_motor,-arm_speed);                        //brings the arm up
      tape_follow();
    }
  }
}
  ...
  delay(100);
   ...
   while(digitalRead(menu_next) != LOW && !startbutton())
     ...
     while(digitalRead(menu_next) != LOW && !startbutton())
    {
      ...
      LCD.home();
      LCD.setCursor(0,0);
      LCD.print("Error 1=" ); LCD.print(error_one); 
      LCD.setCursor(0,1);
      LCD.print("set=change value");
      delay(10);
      if(digitalRead(menu_set) == LOW)
      {
        while(digitalRead(menu_next) != LOW && !startbutton())
        {
          LCD.clear();
          LCD.setCursor(0,0);
          LCD.print("Error 1="); LCD.print(knob(6)/180);
          error_one = knob(6)/180; 
          delay(20);
        }
      }
    }
     ...
     while(digitalRead(menu_next) != LOW && !startbutton())
    {
      ...
      LCD.home();
      LCD.setCursor(0,0);
      LCD.print("Error 2=" ); LCD.print(error_two); 
      LCD.setCursor(0,1);
      LCD.print("set=change value");
      delay(10);
      if(digitalRead(menu_set) == LOW)
      {
        while(digitalRead(menu_next) != LOW && !startbutton())
        {
          LCD.clear();
          LCD.setCursor(0,0);
          LCD.print("Error 2="); LCD.print(knob(6)/120);
          error_two = knob(6)/120; 
          delay(20);
        }
      }
    }
...
void tape_follow()
{
  ...
  if ((left_s>QRD_thresh)&&(right_s<QRD_thresh)) error = -error_one; 
  if ((left_s<QRD_thresh)&&(right_s>QRD_thresh)) error = +error_one; 
